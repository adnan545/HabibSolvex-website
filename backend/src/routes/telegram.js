...
const imageGroups = {};
const GROUP_TIMEOUT = 10000; // 10 seconds
...
async function processImageGroup(mediaGroupId) {
  try {
    ...
    // Create event with all images (stored in MongoDB as Base64)
    const event = await Event.create({
      ...
      images: group.images, // Now stores Base64 data URLs
      files: [],
      isPublished: true
    });
    
    console.log(`✅ Event created with ${group.images.length} images in MongoDB:`, event._id);
  ...
  try {
    ...
    if (body.message) {
      ...
      if (body.message.photo) {
        ...
        if (!base64Image) {
          await sendTelegramMessage('❌ Failed to download image. Please try again.', chatId);
        ...
        console.log('✅ Event created with image in MongoDB:', event._id);
